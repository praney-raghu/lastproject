...
class LanguageLine extends Model
{
    ...
    {
        return Cache::rememberForever(static::getCacheKey($group, $locale, $owner = Neev::organisation()->getKey() ), function () use ($group, $locale) {
            return static::query()
                ->where(['group' => $group, 'owner_id' => Neev::organisation()->getKey() ])
                ->get()
    ...
    public static function getCacheKey(string $group, string $locale, string $owner) : string
    {
        return "spatie.translation-loader.{$group}.{$locale}.{$owner}";
    }
    ...
    protected function flushGroupCache()
    {
        foreach ($this->getTranslatedLocales() as $locale) {
            Cache::forget(static::getCacheKey($this->group, $locale, Neev::organisation()->getKey() ));
        }
    }
